...
Blob<float> *Conv2D::forward(Blob<float> *input)
{
	...
	{
		// setup parameter size information
		int input_size_ = input->c() * input->h() * input->w();
	...
	if (input_ == nullptr || batch_size_ != input->n())
	{
		// initialize input (descriptor / input / batch)
		// resource initialize
		input_ = input;
		...
		// initialize output (calculer output / allouer la mémoire ou reset du pointeur)
		// setting output
		cudnnGetConvolution2dForwardOutputDim(conv_desc_, input_desc_, &output_size_[0], &output_size_[1], &output_size_[2], &output_size_[3]);
		if (output_ == nullptr)
	...
	// avec cuDNN
	cudnnConvolutionForward(cuda_->cudnn(), conv_desc_, &cuda_->one, input_desc_, input_->cuda(), &cuda_->zero, output_desc_, output_->cuda());
	cudnnAddTensor(cuda_->cudnn(), &cuda_->one, bias_desc_, biases_, &cuda_->one, output_desc_, output_->cuda());
...
Blob<float> *Conv2D::backward(Blob<float> *grad_output)
{
	...
	// gradients of biases (Cudnn)
	cudnnConvolutionBackwardBias(cuda_->cudnn(), &cuda_->one, output_desc_, output_, &cuda_->zero, bias_desc_, grad_biases_);
	
	// gradients of weights  (Cudnn)
	cudnnConvolutionBackwardFilter(cuda_->cudnn(), &cuda_->one, input_desc_, input_, output_desc_, output_, conv_desc_, conv_bwd_filter_algo_, d_workspace, workspace_size, &cuda_->zero, filter_desc_, grad_weights_);
